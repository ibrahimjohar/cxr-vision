...
from configs.config import (DATA_RAW, IMAGE_SIZE, BATCH_SIZE, TRAIN_SPLIT, VAL_SPLIT, SEED, NUM_CLASSES)
...
def build_rsna_dataset(data_dir=None):
    ...
    df["path"] = df["patientId"].apply(lambda pid: os.path.join(image_dir, f"{pid}.png"))
...
def _split_data(paths, labels):
    ...
    train_p, temp_p, train_l, temp_l = train_test_split(paths, labels, test_size=(1 - TRAIN_SPLIT), random_state=SEED, stratify=labels)
    # Second split: val vs test from temp
    val_ratio = VAL_SPLIT / (VAL_SPLIT + (1 - TRAIN_SPLIT - VAL_SPLIT))
    val_p, test_p, val_l, test_l = train_test_split(temp_p, temp_l, test_size=0.5, random_state=SEED, stratify=temp_l)
