...
class AttentionGate(nn.Module):
    ...
    the gate learns which spatial regions in the encoder feature map are relevant given the decoder's current state.
    this replaces blind concatenation of encoder features with selectively weighted, context-aware features.
    """
    ...
    def forward(self, g, x):
        ...
        #psi (ψ) is just the name for the attention coefficient map
...
class AttentionUNet(nn.Module):
    def __init__(self, in_channels=1, num_classes=1, features=UNET_FEATURES):
        super().__init__()
